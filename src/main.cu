...
__device__ Color
extract_texture_color(cudaTextureObject_t texture_object, double i, double j, int skybox_width){
    return Color(
...
BlackHole * black_holes, int num_black_holes
    ...
    double skybox_height_coordinate = elevation_angle / 180;
    double skybox_width_coordinate  = azimuth_angle   / 360;
...
*/
    ...
    while(angle < 360){ // TODO?: call multiple kernels from multiple threads
        ...
        double frame_duration = std::chrono::duration<double, std::milli>(frame_end - frame_start).count();
        ...
        double other_duration = std::chrono::duration<double, std::milli>(other_end - other_start).count();
